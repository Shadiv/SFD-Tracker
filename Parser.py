...
# Renaming columns so all column indexes = column names

copies = {}
#there is a lot of similar data inside the table (almost duplicite columns. Parsing doesn't work well enough).
...
for i in tab1:
    ...
    for n in range(len(tab1.columns)):
        if np.sum(np.where(tab1[i] == tab1[n], True, False)) > 3 and i != n:
            dups.append(n)
    # uses loop to identifiy duplicite data between two columns, and if there is more than 3 dups
    # (table still includes some NaN cells, so it cannot be without dups), it appends duplicate column to "dups".
    copies[i] = [*dups]
    #each column is compared with each column
unique_cols = [i for i in range(len(tab1.columns)) if len(copies[i]) < 2 and copies[i] != []]
#list of relatively unique columns (those that have only one duplicate)
